...
from old_profil import Profil
from old_fonctions import *
...
def fonction_recommandation_OLD(profil_id):

    profil = Profil(int(profil_id))

    # Sélection de films à partir des artistes (acteur, scénariste, ...) appréciés par l'utilisateur
    ...
    WHERE _artist.id_artist IN (""" + appreciated_artists + ") LIMIT 500;"

    engine = get_engine()
    mo_req = pd.read_sql(request, engine)

    # Sélection des genres des films ci-dessus

    request = """SELECT DISTINCT ON (_oeuvre.id_oeuvre, genre_name) _oeuvre.id_oeuvre AS idoeuvre, genre_name FROM _oeuvre 
    INNER JOIN _oeuvre_artist ON _oeuvre.id_oeuvre = _oeuvre_artist.id_oeuvre 
    INNER JOIN _artist ON _oeuvre_artist.id_artist = _artist.id_artist 
    ...
    WHERE _artist.id_artist IN (""" + appreciated_artists + ") LIMIT 200;"
    ...
    for I_movie in array_unwatched_movies:
        ...
        for J_movie in array_watched_movies:
            tabTempDistance.append(dis(I_movie["x"], I_movie["y"], J_movie["x"], J_movie["y"]) * (1/J_movie["rating"]))
    ...
    while len(array_ranked_movies) > 0:
        ...
        recommandation_array.append(id_min_distance_temp)

        del array_ranked_movies[id_min_distance_temp]

    return recommandation_array[0:10]

    
def recommandation_OLD(profil_id):
    # Fonction destinée à éviter certaines erreurs de connexion fortuites
    # Répète quatre fois le lancement de la fonction au cas où quatre erreurs de connexion d'affilé se produiraient (ce qui est très peu probable)
    
    i = 0
    while i < 5:
        try:
            data = fonction_recommandation_OLD(profil_id)
            if(data == None): 
                return []
            return data
