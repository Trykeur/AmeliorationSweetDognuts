...
from connect import CreateAccount
...
"""---------------------------------------- RECOMMANDATION ----------------------------------------"""
@app.get("/recommandation/by/id_oeuvre/{id_oeuvre}")
def RecommandationByOeuvre(id_oeuvre,field: List[str] = Query(None),limit = LIMIT):
    """
    You have watched this movie, others have also enjoyed
    EXAMPLE : 
    - http://127.0.0.1:8000/recommandation/by/id_oeuvre/2655467
    """
    try : id_oeuvre = int(id_oeuvre)
    except : return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST, content="'id_oeuvre' must be an integer")
    ...
    data = Recommandation.UserBased_Vecteur(genres,MOVIE,MOVIE_PROFIL,ID_OEUVRE=id_oeuvre,ratingMin=5,seuilScore=0.4)
    response = RequestFilterWithScore(data,'Movie',field,limit)
    return response
...
def recommandationGenre(id_genre,limit = 10):
    ...
    Movies.rename(columns={'genre_name': 'genre'},inplace=True)
